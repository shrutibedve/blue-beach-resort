...
export const GuestActivities: React.FC = () => {
  ...
  const handleRSVP = (evtTitle: string) => {
    setBookingEvent(evtTitle);
    setTimeout(() => {
      setBookingEvent(null);
      alert(`RSVP Confirmed for ${evtTitle}!`);
    }, 1000);
  };
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-100 flex gap-4">
              ...
              <div>
                ...
                <button
                  ...
                  disabled={bookingEvent === evt.title}
                  className="text-xs font-bold text-white bg-slate-900 px-4 py-2 rounded-full hover:bg-slate-800 transition-colors"
                >
                  {bookingEvent === evt.title ? 'Confirming...' : 'RSVP Now'}
                </button>
              </div>
            </div>
          </div>
